...
class EmbeddingBlock(torch.nn.Module):
    def __init__(self, num_radial: int, hidden_channels: int, act: Callable):
        ...
        self.emb = Embedding(125, hidden_channels)
    ...
    def forward(self, x: Tensor, rbf: Tensor, i: Tensor, j: Tensor, h: Tensor) -> Tensor:
        x = self.emb(x)
        if h is not None:
            x = h
...
class DimeNet(torch.nn.Module):
    ...
    def __init__(
        ...
        output_initializer: str = 'zeros',
        # VNODE
        VNODE_Z: Optional[int] = DEFAULT_VNODE_Z,
        ...
        assert self.msg_direction in MSG_DIRECTIONS, f"Unsupported MSG DIRECTION {msg_direction}"
    ...
    def forward(
        ...
        vn_indices = torch.where(z == self.VNODE_Z)[0]
        pn_indices = torch.where(z != self.VNODE_Z)[0]
        ...
        return {
            ...
            "vn_feat": P[vn_indices],
            "pn_feat": P[pn_indices],
        }
