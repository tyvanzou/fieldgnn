...
class FinetuneModelBase(LightningModuleBase[TConfig], Generic[TConfig]):
    ...
    def on_test_end(self):
        ...
        # NOTE MODIFY
        # norm = self.config.normalization.get('y')
        # Path(f'./test_result').mkdir(exist_ok=True, parents=True)
        # self.my_labels = torch.concat(self.my_labels).detach().cpu().numpy()
        # self.my_preds = torch.concat(self.my_preds).detach().cpu().numpy()
        # import pickle
        # import pandas as pd
        # df = pd.DataFrame({
        #     "target": self.my_labels * norm.std + norm.mean,
        #     "predict": self.my_preds * norm.std + norm.mean
        # })
        # df.to_csv(f'./test_result/{self.config.name}.csv', index=False)
        ...
        import pandas as pd
        df = pd.DataFrame({
            "target": self.my_labels,
            "predict": self.my_preds
        })
        df.to_csv(f'./test_result/{self.config.name}.csv', index=False)
