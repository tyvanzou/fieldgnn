...
class GemNetOCBackbone(nn.Module):
    ...
    def symmetrize_edges(self, graph, batch_idx, atomic_numbers):
        """
        ...
        # NOTE MODIFY
        # select real node idx
        # message can not output from the virtual node, thus use idx_src
        # virtual node is represented with atomic_number = 120
        # print("BEFORE FILTER VNODE: ", new_graph['edge_index'].shape, atomic_numbers.shape)
        real_atom_idx = torch.where(atomic_numbers[new_graph["edge_index"][0]] != 120)[
            0
        ]
        new_graph["edge_index"] = new_graph["edge_index"][:, real_atom_idx]
        # print("AFTER FILTER VNODE: ", new_graph['edge_index'].shape)
        new_graph["cell_offset"] = new_graph["cell_offset"][real_atom_idx]
        new_graph["distance"] = new_graph["distance"][real_atom_idx]
        new_graph["vector"] = new_graph["vector"][real_atom_idx]
        ...
        # NOTE MODIFY
        # since symmetric message passing can not be used when virtual nodes is included
        # we set id_swap = None to escape incautious use
    ...
    def get_graphs_and_indices(self, data):
        ...
        if True:
            # NOTE MODIFY
            # add atomic_numbers for real node select
            main_graph, id_swap = self.symmetrize_edges(
                main_graph, data.batch, atomic_numbers=data.atomic_numbers
            )
        else:
