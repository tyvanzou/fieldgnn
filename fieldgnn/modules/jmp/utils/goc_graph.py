...
def _generate_graph(
    ...
    *,
    ...
    max_neighbors: int,
    pbc: bool,
    VNODE_Z: Optional[int],
    msg_direction: Literal['p2p', 'p2v', 'v2v', 'p2pv']
):
    if pbc:
        ...
        # print("DEBUG", edge_index.shape, cell_offsets.shape, neighbors)

        # NOTE MODIFY
        # remove output from virtual node
        assert (x := data["atomic_numbers"]) is not None
        # indices = torch.where(torch.logical_or(x[edge_index[0]] != VNODE_Z, x[edge_index[1]] == VNODE_Z))[0]

        if msg_direction == 'p2p':
            indices = torch.where(torch.logical_and(x[edge_index[0]] != VNODE_Z, x[edge_index[1]] != VNODE_Z))[0]
        ...
        cell_offsets = cell_offsets[indices]
        neighbors[0] = edge_index.shape[1]

        # print("AFTER", edge_index.shape, cell_offsets.shape, neighbors)
...
def generate_graph(
    ...
    *,
    ...
    VNODE_Z: Optional[int] = None,
    msg_direction: Literal['p2p', 'p2v', 'p2pv', 'v2v'] = 'p2pv'
):
    assert msg_direction in ['p2p', 'p2v', 'p2pv', 'v2v']
    (
        edge_index,
        edge_dist,
        distance_vec,
